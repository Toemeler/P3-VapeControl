...
struct ControlView: View {
    ...
    private var powerButton: some View {
        if viewModel.canPowerOven {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(off ? Color.secondary : DS.Palette.accent)
    ...
    private var dialCentre: some View {
        VStack(spacing: 2) {
            ...
            // and a slope through the last twenty.
            if let seconds = viewModel.secondsToReady {
                countdown("ready in \(PaxDeviceViewModel.shortDuration(seconds))")
            } else if viewModel.isCharging == true, let seconds = viewModel.secondsToFull {
                countdown("full in \(PaxDeviceViewModel.shortDuration(seconds))")
            }
        }
        .animation(.easeOut(duration: 0.25), value: viewModel.secondsToReady == nil)
    }

    private func countdown(_ text: String) -> some View {
